...
{
	auto* writer = static_cast<JsonWriter*>(stream);
...
JsonWriter::JsonWriter(URIMap& map, URIs& uris, URI uri)
    ...
    , _uri{std::move(uri)}
{
	...
	// Make a Json writer that writes to text_sink
...
bool
JsonWriter::write(const LV2_Atom* msg, int32_t)
{
